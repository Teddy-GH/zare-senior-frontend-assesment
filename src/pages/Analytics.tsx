...
const calculateWorkloadDistribution = (teamMembers: TeamMember[], projects: Project[]): WorkloadData[] => {
  ...
  teamMembers.forEach(member => {
    workloadMap.set(member.id, { workload: 0, count: 0 });
  });

  // Calculate workload for each project assignment
  // Assuming projects have a 'team' field that references the team member ID
  projects.forEach(project => {
    const weight = priorityWeights[project.priority] || 1;
    
    // If project has a single team member assignment
    if (project.team) {
...
const calculateStandardDeviation = (workloads: number[]): number => {
...
// Complexity: O(n^2 * m) - evaluates all possible reassignments
const generateReassignmentSuggestions = (
  ...
  overloaded.forEach(overMember => {
    // Find projects assigned to overloaded member
    const memberProjects = projects.filter(project =>
      project.team === overMember.memberId
    );
    
    balanced.forEach(balMember => {
      // Find a project to reassign
      const candidateProject = memberProjects.find(project => {
        // Check if project not already assigned to balanced member
        return project.team !== balMember.memberId;
      });
...
const clusterProjectsByWeek = (projects: Project[]): DeadlineCluster[] => {
  ...
  weekMap.forEach((weekProjects, weekKey) => {
    ...
    // Calculate total members needed (assuming 1 team member per project)
    const totalMembersNeeded = weekProjects.length;
    
    // Calculate risk score
    let riskScore = 0;
    riskScore += projectCount * 2; // Each project adds 2 points
    riskScore += totalMembersNeeded > 5 ? 3 : 0; // High resource need
